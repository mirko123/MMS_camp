...
static int digit(void *_ctx, int cur_state, int event, int next_state) {
	...
	ctx->accumulator = ctx->accumulator * 16 + val;
...
void do_hex_calc(void) {
	...
	while ((ctx.symbol = fgetc(stdin)) != EOF) {
		if (isdigit(ctx.symbol) ||
			(ctx.symbol == (int)'a') || (ctx.symbol == (int)'b') ||
			(ctx.symbol == (int)'c') || (ctx.symbol == (int)'d') ||
			(ctx.symbol == (int)'e') || (ctx.symbol == (int)'f')) event = EVENT_DIGIT;
